...
**해결 안** : settime을 사용하여 메세지가 출력된후 confirm이 나오도록 수정함

## 6. 한계점
**쉬운 개념의 모호함**: 쉬운 개념을 설명하기가 어려운 점 예를 들어 볼펜을 설명하려고 할때 볼펜을 다 알고있는 대화내용이 출력됨
**반복 질문의 답변 통일성** : 같은 개념에 대해 질문자가 알려주려고 시도할 ~~때~~, 답변이 통일되지 않은 모습을 보임 예를들어 볼펜을 아예 모른다고 답하거나 볼펜을 거의 90% 알고 있다고 말함

## 7. 결론
Feynman Study Model 웹 애플리케이션은 사용자가 복잡한 개념을 더 쉽게 이해하고 설명할 수 있도록 도와줍니다. AI와의 상호작용을 통해 사용자는 자신의 설명 능력을 검증하고 개선할 수 있으며, 이는 효과적인 학습과 이해를 촉진합니다. 이 프로젝트는 HTML, CSS, JavaScript, Chat GPT를 활용하여 구현되었으며, Feynman 공부법의 원칙을 실천하며, 자신의 설명 능력을 향상시키고, 학습 효과를 높일 수 있습니다. 
